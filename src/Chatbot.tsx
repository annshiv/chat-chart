...
import './App.css';
import { ETemplateId, IState, initialState, setTemplate } from './redux/templateSlice';
...
interface IChatBot {
  ...
  config: IState;
  setConfig: React.Dispatch<React.SetStateAction<IState>>;
}
...
const Chatbot = (props: IChatBot) => {
  const { showChart, setShowChart, setConfig, config } = props;
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const aiResponse: Message = { user: false, text: 'done' };
    ...
    if (res.status === 200) {
      ...
      setMessage('');
      setConfig({ ...initialState, ...parsedResponse });
